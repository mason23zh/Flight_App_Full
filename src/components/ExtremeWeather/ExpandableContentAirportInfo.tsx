...
function ExpandableContentAirportInfo({ row, airportData }) {
  ...
  const renderWindComponentButtons = (windDegrees, windSpeed, runwayHdg) => {
    ...
    const filteredWindSpeed =
      typeof windSpeed === "number"
        ? windSpeed
        : Number(windSpeed.replace(/[^0-9]/g, ""))
    const crossWind = Math.round(
      Math.sin(toRadians(Number(runwayHdg) - Number(windDegrees))) *
        filteredWindSpeed
    )
    const headWind = Math.round(
      Math.cos(toRadians(Number(runwayHdg) - Number(windDegrees))) *
        filteredWindSpeed
    )
  ...
  const renderedRunways = airportData.runways.map((runways) => (
    <div key={runways.runway_id} className="p-3 flex flex-col ">
      ...
      <div>
        {runways.thresholdOverflyAlt
          ? `TCH: ${runways.thresholdOverflyAlt} ft`
          : ""}
      </div>
      <div>
        {runways.thresholdElevation
          ? `Elevation: ${runways.thresholdElevation} ft`
          : ""}
      </div>
  ...
  const handleClick = () => {
    ...
    router.push("/airport/detail?icao="+ airportData.ICAO)
  }

  return (
    <div className="text-sm p-3 md:text-lg">
      <div className="flex flex-col justify-center items-center">
        <div>{row.original.raw_text}</div>
        <div>{airportElevation ? `Elevation: ${airportElevation} ft` : ""}</div>
        <div>
          {airportTransAltitude
            ? `Transition Altitude: ${airportTransAltitude} ft`
            : ""}
        </div>
        <div>
          <button
