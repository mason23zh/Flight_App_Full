...
function VatsimAirportInfoTab({ airport, counter }) {
    const {
        ICAO, iata, station, arrivalNumber, departureNumber, controller,
    } = airport;
    ...
    const gndIcon = controller.GND ? (
        <div
            className="border-2 bg-green-500 text-center"
            title="Ground"
        >
    ...
    const goToAirportIcon = (
        <Link
            // href={`airport/${ICAO}`}
            href={{ pathname: `/airport/detail`, query: { code: ICAO } }}
            // href={'/airport/detail'}
    ...
    return (
        <div className={themeClass}>
            ...
            <div className="justify-self-start p-2 ml-1 md:ml-0">
                {icaoSection}
            </div>
            ...
            <div className="p-2 hidden sm:block">
                {controllerIconsSection}
            </div>
            <div className="p-2 md:mr-4 justify-self-center">
                {trafficSections}
            </div>
