...
function VatsimEventsListItem({ event, onClick }: Props) {
    const darkMode = useTheme();
    const dispatch = useDispatch();
    const e: PartialEvent = useSelector<RootState>(
    ...
    const generateTheme = (storeEvent: PartialEvent, currentEvent: PartialEvent) => {
        ...
        if (darkMode && storeEvent.id === currentEvent.id) {
            generatedTheme =
                "grid grid-cols-1 p-3 border-2 bg-indigo-500 rounded-xl relative " +
            ...
            generatedTheme =
                "grid grid-cols-1 p-3 border-[1px] bg-gray-500 rounded-xl relative " +
                "transition ease-in-out delay-50 bg-blue-500 hover:-translate-y-1 " +
                "hover:scale-100 hover:bg-indigo-400 duration-300";
        } else if (!darkMode && storeEvent.id === currentEvent.id) {
            generatedTheme =
                "grid grid-cols-1 p-3 border-2 bg-indigo-400 rounded-xl relative " +
            ...
            generatedTheme =
                "grid grid-cols-1 p-3 border-2 bg-gray-300 rounded-xl relative " +
                "transition ease-in-out delay-50 bg-blue-500 hover:-translate-y-1 " +
        ...
        renderAirportList = event.airports.map((airport) => (
            <div key={Math.random()}>
                <div className={airportIconStyle}>
                    <Link to={`/airport/detail/${airport.icao}`}>{airport.icao}</Link>
                </div>
            </div>
        ));
    }
    // top-0 right-0 translate-x-[-300%]
    const renderInProgress = (startTime, endTime) =>
        checkCurrent(startTime, endTime) ? (
    ...
    return (
        <div className={generateTheme(e, event)} onClick={handleClick}>
            ...
            <div className="justify-self-start">{renderTime(start_time, end_time, true)}</div>
            <div className="justify-self-start">
                <div className="flex gap-1 flex-wrap">{renderAirportList}</div>
            </div>
