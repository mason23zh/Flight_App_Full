...
import { VatsimFss } from "../../../../types";
import { useTheme } from "../../../../hooks/ThemeContext";
import { Popup } from "react-map-gl";

interface Props {
    hoverFss: GeoJson.FeatureCollection,
    fssData: VatsimFss
}
...
const FssLabelPopup = ({
    ...
    const firName = hoverFss.features[0].properties.firInfo.name;
    ...
    const renderControllersData = hoverFss.features[0].properties.controllers.map((c) => {
        const {
            hour,
            minute
    ...
    return (
        <Popup
            ...
            maxWidth="500px"
            longitude={Number(hoverFss.features[0].properties.label_lon)}
            latitude={Number(hoverFss.features[0].properties.label_lat)}
            closeButton={false}
            anchor="bottom"
        >
            ...
            <div className={`w-full p-2 font-Rubik rounded-xl ${colorTheme}`}>
                <div className="flex text-center gap-3 justify-self-start w-full">
                    <div className="text-[17px] font-bold">
                        {hoverFss.features[0].properties.id}
                    </div>
