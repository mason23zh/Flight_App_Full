...
export const extractTraconName = (controller: Controller) => {
    ...
    if (!traconName && airport) {
        const airportName = airport.name.replace("Airport", "")
            .trim();
        if (callsign.includes("APP")) {
            traconName = `${airportName} Approach`;
        } else if (callsign.includes("ARR") || callsign.includes("DEP") || callsign.includes("Arrival") || callsign.includes("Departure")) {
