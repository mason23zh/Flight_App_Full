...
function HomeVatsimEventInfoTab({ event, counter }) {
    const { end_time, start_time, link, name } = event;
    ...
    const nameSection = <div className="items-center">{name ? <div>{name}</div> : <></>}</div>;

    const timeSection = (
        <div>
            {start_time && end_time ? (
                <div>
                    {moment(start_time).utc().format("HH:mm")} to{" "}
                    {moment(end_time).utc().format("HH:mm")} (UTC)
                </div>
            ) : (
                <></>
            )}
    ...
    const goToSection = (
        <Link to={`/vatsim/events/${name}`} onMouseOver={handleClick}>
    ...
    return (
        <div className={themeClass}>
            ...
            <div className="p-2 ">{timeSection}</div>
            <div className="p-2 justify-self-center hidden sm:block">{linkSection}</div>
            <div className="justify-self-end mr-3">{goToSection}</div>
        </div>
    );
}
