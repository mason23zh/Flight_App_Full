        ...
        return await db.airports
            .filter(
                (airport) =>
                    ...
                    (airport.gps_code &&
                        airport.gps_code.toLowerCase().includes(query.toLowerCase()))
            )
            .toArray();
    } catch (e) {
...
export const searchAirportByIdent = async (query: string): Promise<LocalDbAirport[]> => {
    ...
    try {
        return await db.airports
            .filter((airport) => airport.ident.toLowerCase().includes(lowerCaseQuery))
            .toArray();
    } catch (e) {
        return [];
    }
...
export const searchVatsimTraffic = async (query: string): Promise<VatsimFlight[]> => {
    ...
    try {
        return await db.vatsimTraffic
            .filter(
                (traffic) =>
                    traffic.callsign.toLowerCase().includes(lowerCaseQuery) ||
                    traffic.cid.toString().includes(lowerCaseQuery) ||
                    (traffic?.flight_plan?.aircraft?.toLowerCase() || "").includes(
                        lowerCaseQuery
                    ) ||
                    (traffic?.flight_plan?.aircraft_faa?.toLowerCase() || "").includes(
                        lowerCaseQuery
                    ) ||
                    (traffic?.flight_plan?.aircraft_short?.toLowerCase() || "").includes(
                        lowerCaseQuery
                    ) ||
...
const groupByAircraftType = (flights: VatsimFlight[]): GroupedFlight[] => {
    ...
    return Object.keys(groupedFlights).map((aircraftType) => ({
        aircraftType,
        ...
        const results = await db.vatsimTraffic
            .filter(
                (traffic) =>
                    ...
                    (traffic?.flight_plan?.aircraft_iata?.toLocaleLowerCase() || "").includes(
                        lowerCaseQuery
                    ) ||
                    (traffic?.flight_plan?.aircraft_name?.toLocaleLowerCase() || "").includes(
                        lowerCaseQuery
                    )
            )
            .toArray();
...
export const searchFlightsByAirports = async (query: string): Promise<VatsimFlight[]> => {
    ...
    try {
        return await db.vatsimTraffic
            .filter(
                (traffic) =>
                    (traffic?.flight_plan?.departure.toLowerCase() || "").includes(
                        lowerCaseQuery
                    ) ||
                    (traffic?.flight_plan?.arrival.toLowerCase() || "").includes(lowerCaseQuery)
            )
