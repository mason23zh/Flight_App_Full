...
const AirportInfoExpandContent_Weather = ({
    ...
    if (airportError || tafError) {
        return <div>Unable to fetch weather for {airport.ident.toUpperCase()}</div>;
    }
    ...
    const renderRawMetarText = (airportData: DetailAirportResponseQuery) => {
        if (airportData.data && airportData.result !== 0) {
            return <div>{airportData.data[0].METAR?.raw_text || "N/A"}</div>;
        }

        return <div>No METAR available</div>;
    };

    const renderRawTafText = (taf) => {
        if (taf.data && taf.results !== 0) {
            return <div>{taf.data[0]}</div>;
        }
        return <div>No TAF available</div>;
    };
        ...
        return (
            <div className="p-1 rounded-lg max-h-full overflow-y-auto">
                <div className="p-1 grid grid-cols-1 text-sm">
                    <div className="font-bold">METAR</div>
                    <div>{renderRawMetarText(airportData)}</div>
                </div>
                <div className="p-1 grid grid-cols-1 text-sm">
                    <div className="font-bold">TAF</div>
                    <div>{renderRawTafText(taf)}</div>
                </div>
            </div>
        );
    }
