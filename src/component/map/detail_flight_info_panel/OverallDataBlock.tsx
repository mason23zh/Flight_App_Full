    ...
    return (
        <div className="relative">
            ...
            <div className="grid grid-rows-2">
                <div
                    className={`grid ${airlinerInfo ? "grid-rows-3" : "grid-rows-2"} bg-gray-600 p-2`}
                >
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            {trackTooltipVisible && !isTouchScreen && (
                                ...
                                >
                                    <div>Move to traffic</div>
                                </div>
                            )}
                        </div>
                    </div>
                    {airlinerInfo && (
                        <div className="text-white text-xs md:text-sm flex items-center gap-1">
                            {airlinerInfo.name}
                                ...
                                {tooltipVisible && !isTouchScreen && (
                                    ...
                                    >
                                        <div className="flex items-center gap-1">
                                            <div>Callsign:</div>
                                            <div>{airlinerInfo?.callsign}</div>
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <div>Country:</div>
                                            <div>{airlinerInfo?.country}</div>
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="text-white text-xs md:text-sm">{aircraft || "N/A"}</div>
                </div>
                    ...
                    <div className="flex flex-col justify-center items-center border-r p-2 md:p-3 bg-gray-300">
                        <div className="text-lg md:text-xl font-bold">
                            {depAirport.data[0]?.ICAO ? (
                                <Link
                                    to={`/airport/detail/${depAirport.data[0].ICAO}`}
                        ...
                        <div
                            className="text-xs md:text-lg truncate w-full text-center"
                    ...
                    <div className="flex flex-col justify-center items-center border-l p-3 bg-gray-300">
                        <div className="text-lg md:text-xl font-bold">
                            {arrAirport.data[0]?.ICAO ? (
                                <Link
                                    to={`/airport/detail/${arrAirport.data[0].ICAO}`}
                                    className="text-blue-600
                ...
                <div className="grid grid-cols-2 font-bold font-Rubik">
                    ...
                    <div className="flex justify-between bg-gray-400 p-2 border-l text-gray-900">
                        <div>SCHEDULE</div>
                        <div>{etd && enroute ? calculateArrivalTime(etd, enroute) : "N/A"}</div>
                    </div>
                </div>
            </div>
            <div className="bg-gray-300 p-2">
                <div className="grid grid-cols-[auto,1fr,auto] items-center font-bold font-Rubik">
                    <div className="whitespace-nowrap">{depAirport.data[0]?.iata || "N/A"}</div>
                    <div className="mx-2">
                        {progress !== -1 ? (
                            <FlightProgressBar progress={progress} />
                        ) : (
                            <div className="text-center">Progress N/A</div>
                        )}
                    </div>
                    <div className="whitespace-nowrap">{arrAirport.data[0]?.iata || "N/A"}</div>
                </div>
            </div>
        </div>
    );
